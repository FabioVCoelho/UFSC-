...
PUBLIC ssize_t file_read(struct inode *i, void *buf, size_t n, off_t off)
{
	...
	do
	{
		...
		// Prefetch control
		int seq = ((off - last_off) == BLOCK_SIZE);
		if (seq) {
			if (prefetch) 
				pf_ctrl =  PF_THRESHOLD;
			else 
				pf_ctrl = pf_ctrl > PF_THRESHOLD - 1 ? PF_THRESHOLD : pf_ctrl + 1;
		}
		else {
			if (!prefetch)
				pf_ctrl = 0;
			else
			pf_ctrl = pf_ctrl < 1 ? 0 : pf_ctrl - 1;
		}
		prefetch = (prefetch && (pf_ctrl > 0)) || (pf_ctrl == PF_THRESHOLD);
		last_off = off;

		if (prefetch) 
		{
			block_t blk2 = block_map(i, off + BLOCK_SIZE, 0);
			if (blk2 != BLOCK_NULL)
			{
				struct buffer *bbuf2 = bread_async(i->dev, blk2);
				brelse(bbuf2);
			}
		}
