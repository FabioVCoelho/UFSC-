...
PRIVATE struct {
    ...
    unsigned marked; /**< Already visited.    */
    unsigned lastReferenceTime; /**< Last reference time  */
} frames[NR_FRAMES] = {
    {0, 0, 0, 0, 0, 0},
};
...
PRIVATE int allocf(void) {
    ...
    int fullturn = 0;
    /*
     * Se já deu 2 voltas completas pelos processos e não achou ninguem
     * manda o primeiro frame.
     */
    while (fullturn < 2) {
        unsigned swap_out_marked = 0;
        ...
        for (x = 0; x <= NR_FRAMES; x++) {
            ...
            // How many frames this process have on total.
            unsigned processWithFramesInTotal = 0;

            // How many frames this process have marked.
            unsigned processWithFramesMarked = 0;

            /*
             * Simula o ponteiro do relogio.
             * Para cada frame que o processo possui adiciona a uma variavel
             * Se o frame está marcado, este já foi visitado nessa volta, passa para o próximo.
             * Se o frame não está marcado, manda para a verificação dos bits do frame.
             */

            // Sem o if perde a politica local, mas com if dá core dump

            // if (frames[i].owner == curr_proc->pid) {
            //     kprintf("Frames com owner == pid  %d",i);

            processWithFramesInTotal++;
            if (frames[x].marked == 1) {
                processWithFramesMarked++;
            } else {
                frames[x].marked = 1;
                addr_t addr = frames[x].addr;
                struct pte *pg = getpte(curr_proc, addr);
                unsigned r = pg->accessed;

                /*
                 * Se R = 1, ela foi referenciada antes da interrupção de 
                 * relógio atual, então não é candidata a remoção
                 */

                if (r == 1) {
                    // Zera o bit R.
                    pg->accessed = 0;
                    // Copia o tempo virtual atual para o instante do último uso;
                    frames[x].lastReferenceTime = curr_proc->vt;
                } else {
                    frames[x].age = curr_proc->vt - frames[x].lastReferenceTime;
                    // Se idade > T e M = 0 , substitui a página
                    // idade apresentando problema.
                    // frames[i].age >= THETA &&
                    if (pg->dirty == 0) {
                        frames[x].count = 1;
                        kprintf("encontrei antes do while!! %d", x);
                        return (x);
                    }/*
                             * Se idade > T e M = 1 , escalona a escrita da 
                             * página no disco e avança o ponteiro 
                             * (ao final da escrita: M = 0 ).
                             *  
                             */
                        // idade apresentando problema.
                        // frames[i].age >= THETA &&
                    else if (pg->dirty == 1) {
                        kprintf("escalonei antes do while!! %d", x);
                        if (swap_out(curr_proc, addr)) {
                            return -1;
                        }
                        swap_out_marked++;
                        continue;
            ...
            /*
             * Se for o ultimo frame a vistar, deu uma volta completa.
             * Se nesta volta completa, todos os frames estão marcados,
             * desmarca todos os frames e inicializa novamente.
             */
            if (x == NR_FRAMES) {
                int j = 0;
                fullturn++;
                if (processWithFramesMarked == processWithFramesInTotal)
                    for (j = 0; j <= NR_FRAMES; j++)
                        //if (frames[j].owner == curr_proc->pid) {
                        frames[j].marked = 0;
                // }
                /*
                 * Se pelo menos uma escrita foi escalonada , o ponteiro poderá 
                 * encontrar uma página “limpa” (M = 0 ), logo, a removerá
                 */
                if (swap_out_marked > 0)
                    continue;
                else
                    break;
            }
    ...
    while (1) {
        int x1;
        for (x1 = 0; x1 <= NR_FRAMES; x1++) {
            addr_t addr1 = frames[x1].addr;
            struct pte *pg1 = getpte(curr_proc, addr1);
            if (pg1->dirty == 0) {
                frames[x1].marked = 1;
                frames[x1].count = 1;
                kprintf("encontrei!! %d", x1);
                return (x1);
            } else {
                kprintf("escalonandoooo! %d", x1);
                if (swap_out(curr_proc, addr1)) {
                    return -1;
                }
            }
        }
    ...
    kprintf("Não sei como chegou aqui kkkk");
...
PUBLIC void freeupg(struct pte * pg) {
    ...
    /* Double free. */
    if (frames[i].count == 0) {
        kprintf("Frame with count = 0 %d", i);
