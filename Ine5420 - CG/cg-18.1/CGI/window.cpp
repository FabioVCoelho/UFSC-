...
void Window::rotate(angle_t angle) {
	matrix3f rot = rotate3f(angle);
	rot >> m_up;
	rot >> m_right;
	updateTransforms();
}

void Window::rotate(const matrix3f& rotation) {
	rotation >> m_up;
	rotation >> m_right;
	updateTransforms();
}
...
const Vec2f& Window::center() const {
	return m_center;
}
...
points_t Window::cornersWorldCoords() {
	Vec2f p0 = m_center + m_right*(m_width / 2) + m_up*(m_height / 2);
	Vec2f p1 = p0 - m_up*m_height;
	Vec2f p2 = p1 - m_right*m_width;
	Vec2f p3 = p0 - m_right*m_width;
	return { p0, p1, p2, p3 };
}
