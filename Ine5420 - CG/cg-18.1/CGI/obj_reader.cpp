...
shape_ptr_t parseCurve(std::istringstream& ss, const points_t& vertices, Color color) {
	...
	curva2 curve(vertices[indexes[0] - 1], vertices[indexes[1] - 1], vertices[indexes[2] - 1], vertices[indexes[3] - 1], color);
	return curve.clone();
}

shape_ptr_t parseShape(std::istringstream& ss, const points_t& vertices, Color color, bool fill, bool closed) {
	std::vector<size_t> indexes;
	size_t i;
	while (ss >> i) {
		indexes.push_back(i);
...
) 
{
	...
	while (std::getline(fs, line)) {
		...
		case 'l':
			if (curve) {
				control.addShapeInit(objname, *parseCurve(ss2, vertices, color));
			}
			else {
				control.addShapeInit(objname, *parseShape(ss2, vertices, color, fill, closed));
			}
			curve = false;
			break;
		case 'w':
