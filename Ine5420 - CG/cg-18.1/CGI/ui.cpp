...
extern "C" EXPORT gboolean on_minimap_draw(GtkWidget *widget, cairo_t *cr, gpointer data) {
	...
	for (auto& drawable : *minimapShapes) {
...
extern "C" EXPORT void on_buttonClickedShowLine(GtkButton *button, gpointer data) {
	...
	Line line(point_t(field1, field2), point_t(field4, field5));
...
extern "C" EXPORT void on_buttonClickedPolygonAddLine(GtkButton *button, gpointer data) {
	pointsPolygon.push_back(point_t(field1, field2));
}
...
extern "C" EXPORT void on_buttonClickedOkCurve(GtkButton *button, gpointer data) {
	...
	ctrl_ptr->addShape(objName, curva2({field1, field2}, { field4, field5 }, { field7, field8 }, { field10, field11 }));
	addObjectToTreeView(objName.c_str(), "Curve");
	hideWindow(data);
}

extern "C" EXPORT void on_buttonClickedAddControlPointBspline(GtkButton *button, gpointer data) {
	pointsControl.push_back(point_t(field1, field2));
}

extern "C" EXPORT void on_buttonClickedOkCurveBSpline(GtkButton *button, gpointer data) {
	std::string objName(field0);
	...
	if (ctrl_ptr->objectExists(objName)) {
        ...
        if (pointsControl.size() > 2) {
            ctrl_ptr->addShape(objName, Bspline(pointsControl));
            std::vector<point_t> pointsControl;
            addObjectToTreeView(objName.c_str(), "BSpline");
            hideWindow(data);
	}
}
