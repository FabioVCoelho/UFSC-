...
const shape_ptr_t CGSys::addShape(std::string name, const Shape& shape) {
	...
	ptr->name(name);
	m_mapShapes.emplace_back(Drawable(edgevertex2f_t(), shape.color(), shape.isPolygon()));
	m_minimapShapes.emplace_back(Drawable(edgevertex2f_t(ptr->points()), shape.color(), shape.isPolygon()));
	updateViewport(ptr);
	return ptr;
}
...
//void CGSys::clip(const shape_ptr_t& shape) {
//	m_clipper.clip(shape, m_window);
//}
...
void CGSys::updateViewport(const shape_ptr_t& shape) {
	...
	//m_mapShapes[shape->id()] = { m_clippedShape.edges().begin(), m_clippedShape.edges().end()};
	//winVpTransf(m_winvpMap, m_clippedShape.m_clippedPoints, m_mapShapes[shape->id()]);
	m_minimapShapes[shape->id()].shape.copy(shape->points());
	m_winvpMinimap >> m_minimapShapes[shape->id()].shape.vertices();
	//m_minimapShapes[shape->id()] = winVpTransf(m_winvpMinimap, shape->points(), m_minimapShapes[shape->id()])
	//winVpTransf(m_winvpMinimap, shape->points(), m_minimapShapes[shape->id()]);
}

void CGSys::updateMinimapWindow() {
	m_minimapWindow.copy(m_window.cornersWorldCoords());
	m_winvpMinimap >> m_minimapWindow.vertices();
	/*vp_points_t mmwVp = winVpTransf(m_winvpMinimap, m_window.cornersWorldCoords());
	size_t size = mmwVp.size();
	for (size_t i = 0; i < size; ++i) {
		m_minimapWindow.emplace_back(vp_edge_t(mmwVp[i], mmwVp[(i + 1 % size)]));
	}*/
	//m_minimapWindow = winVpTransf(m_winvpMinimap, m_window.cornersWorldCoords());
}
