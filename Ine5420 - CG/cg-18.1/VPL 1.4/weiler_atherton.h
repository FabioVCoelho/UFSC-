...
public:
	...
	struct WAVertex {
		WAVertex(const window_point_t& p)
			: m_point(p), m_visited(false), m_next(nullptr) {}

		WAVertex(const window_point_t& p, const pplist::iterator& it)
			: m_point(p), m_visited(false), m_next(nullptr), m_nextinpolylist(it) {}

		const window_point_t& point() const { return m_point; }
	...
	private:
		window_point_t m_point;
		WAVertex* m_next;
		bool m_visited;
		pplist::iterator m_nextinpolylist;
	};

	size_t clip(const edgevertex2f_t& windowShape, edgevertex2f_t& clippedShape);
