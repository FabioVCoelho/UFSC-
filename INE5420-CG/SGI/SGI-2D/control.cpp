...
{}
 ...
 void Control::addShapeInit(std::string name, const Shape& shape) {
	 m_worldObjects[name] = m_cg.addShapeInit(name, shape);
 }
...
std::vector<Drawable>* Control::mapShapes() {
	 //m_cg.updateViewports();
	 return &m_cg.mapShapes();
 }

std::vector<Drawable>* Control::minimapShapes() {
	 //m_cg.updateViewports();
	 return &m_cg.minimapShapes();
 }

edgevertex2f_t* Control::minimapWindow() {
 ...
 std::vector<std::pair<std::string, std::string >> Control::fromObj() {
	...
	for (const auto& pair : m_worldObjects) {	 	  	 	     	   		   	      	 	    	 	
		objs.push_back({ pair.first, pair.second->points().size() > 2 ? "Wireframe" : "Line" });
}
	return objs;
}