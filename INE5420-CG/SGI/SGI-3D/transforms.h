...
template<typename T>
struct Scale4 : public Matrix<4, T> {
	Scale4() {}
	Scale4(double sx, double sy, double sz) {
		Matrix<4, T>::m_data[0][0] = sx;
		Matrix<4, T>::m_data[1][1] = sy;
		Matrix<4, T>::m_data[2][2] = sz;
		Matrix<4, T>::m_data[3][3] = 1;
	}
};


template<typename T>
struct Shift4 : public Matrix<4, T> {
	Shift4() {}
	Shift4(T sx, T sy, T sz) {
		Matrix<4, T>::m_data[0][0] = Matrix<4, T>::m_data[1][1] = Matrix<4, T>::m_data[2][2] = Matrix<4, T>::m_data[3][3] = 1;
		Matrix<4, T>::m_data[0][3] = sx;
		Matrix<4, T>::m_data[1][3] = sy;
		Matrix<4, T>::m_data[2][3] = sz;
	}	 	  	 	     	   		   	      	 	    	 	
	Shift4(const Vec<3, T>& vec) {
		Matrix<4, T>::m_data[0][0] = Matrix<4, T>::m_data[1][1] = Matrix<4, T>::m_data[2][2] = Matrix<4, T>::m_data[3][3] = 1;
		Matrix<4, T>::m_data[0][3] = vec[0];
		Matrix<4, T>::m_data[1][3] = vec[1];
		Matrix<4, T>::m_data[2][3] = vec[2];
	}
};

template<typename T>
struct ScaleCenter4 : public Matrix<4, T> {
	ScaleCenter4() {}
	ScaleCenter4(double sx, double sy, double sz, const Vec<3, T>& center) {
		Matrix<4, T>::m_data[0][0] = sx;
		Matrix<4, T>::m_data[1][1] = sy;
		Matrix<4, T>::m_data[2][2] = sz;
		Matrix<4, T>::m_data[3][3] = 1;
		Matrix<4, T>::m_data[0][3] = center[0] * (1 - sx);
		Matrix<4, T>::m_data[1][3] = center[1] * (1 - sy);
		Matrix<4, T>::m_data[2][3] = center[2] * (1 - sz);
	}
};

template<typename T>
struct XRotate4 : public Matrix<4, T> {
	XRotate4() {}
	XRotate4(angle_t angle) {
		double s = sin(angle);
		double c = cos(angle);
		Matrix<4, T>::m_data[0][0] = 1;
		Matrix<4, T>::m_data[1][1] = c;
		Matrix<4, T>::m_data[2][2] = c;
		Matrix<4, T>::m_data[3][3] = 1;
		Matrix<4, T>::m_data[1][2] = -s;
		Matrix<4, T>::m_data[2][1] = s;
	}
};

template<typename T>
struct YRotate4 : public Matrix<4, T> {	 	  	 	     	   		   	      	 	    	 	
	YRotate4() {}
	YRotate4(angle_t angle) {
		double s = sin(angle);
		double c = cos(angle);
		Matrix<4, T>::m_data[0][0] = c;
		Matrix<4, T>::m_data[1][1] = 1;
		Matrix<4, T>::m_data[2][2] = c;
		Matrix<4, T>::m_data[3][3] = 1;
		Matrix<4, T>::m_data[0][2] = s;
		Matrix<4, T>::m_data[2][0] = -s;
	}
};

template<typename T>
struct ZRotate4 : public Matrix<4, T> {
	ZRotate4() {}
	ZRotate4(angle_t angle) {
		double s = sin(angle);
		double c = cos(angle);
		Matrix<4, T>::m_data[0][0] = c;
		Matrix<4, T>::m_data[1][1] = c;
		Matrix<4, T>::m_data[2][2] = 1;
		Matrix<4, T>::m_data[3][3] = 1;
		Matrix<4, T>::m_data[0][1] = -s;
		Matrix<4, T>::m_data[1][0] = s;
	}
};

template<typename T>
struct XRotateCenter4 : public Matrix<4, T> {
	XRotateCenter4() {}
	XRotateCenter4(angle_t angle, const Vec<3, T>& center) {
		double s = sin(angle);
		double c = cos(angle);
		Matrix<4, T>::m_data[0][0] = 1;
		Matrix<4, T>::m_data[1][1] = c;
		...
		Matrix<4, T>::m_data[1][2] = -s;
		Matrix<4, T>::m_data[2][1] = s;
		Matrix<4, T>::m_data[1][3] = center[1] * (1 - c) + center[2] * s;
		Matrix<4, T>::m_data[2][3] = -center[1] * s + center[2] * (1 - c);
	}	 	  	 	     	   		   	      	 	    	 	
};

template<typename T>
struct YRotateCenter4 : public Matrix<4, T> {
	YRotateCenter4() {}
	YRotateCenter4(angle_t angle, const Vec<3, T>& center) {
		double s = sin(angle);
		double c = cos(angle);
		Matrix<4, T>::m_data[0][0] = c;
		Matrix<4, T>::m_data[1][1] = 1;
		...
		Matrix<4, T>::m_data[0][2] = s;
		Matrix<4, T>::m_data[2][0] = -s;
		Matrix<4, T>::m_data[0][3] = center[0] * (1 - c) - center[2] * s;
		Matrix<4, T>::m_data[2][3] = center[0] * s + center[2] * (1 - c);
	}
};

template<typename T>
struct ZRotateCenter4 : public Matrix<4, T> {
	ZRotateCenter4() {}
	ZRotateCenter4(angle_t angle, const Vec<3, T>& center) {
		double s = sin(angle);
		double c = cos(angle);
		Matrix<4, T>::m_data[0][0] = c;
		Matrix<4, T>::m_data[1][1] = c;
		...
		Matrix<4, T>::m_data[0][1] = -s;
		Matrix<4, T>::m_data[1][0] = s;
		Matrix<4, T>::m_data[0][3] = center[0] * (1 - c) + center[1] * s;
		Matrix<4, T>::m_data[1][3] = -center[0] * s + center[1] * (1 - c);
	}
};
...
using shift4f = Shift4<double>;
using scale4f = Scale4<double>;
using scalec4f = ScaleCenter4<double>;
using xrotate4f = XRotate4<double>;
using yrotate4f = YRotate4<double>;
using zrotate4f = ZRotate4<double>;
using xrotatec4f = XRotateCenter4<double>;
using yrotatec4f = YRotateCenter4<double>;
using zrotatec4f = ZRotateCenter4<double>;
