...
public:
	...
	Vec(const Vec<2, T>& v2) : m_x(v2[0]), m_y(v2[1]), m_z(T()) {}
	template <typename U>
...
Vec<3, T> operator-(const Vec<3, T>& vec) {
	return (Vec<3, T>(-vec.m_x, -vec.m_y, -vec.m_z));
}

template<typename T>
Vec<3, T> operator+(const Vec<3, T>& lhs, const Vec<3, T>& rhs) {
	return (Vec<3, T>(lhs.m_x + rhs.m_x, lhs.m_y + rhs.m_y, lhs.m_z + rhs.m_z));
}
...
U dot(const Vec<3, T>& lhs, const Vec<2, U>& rhs) {
	return U(lhs.m_x*rhs.m_x + lhs.m_y*rhs.m_y + lhs.m_z);
}
