...
PRIVATE void do_clock()
{
	...
	/*
	 * Quando o processo é escalonado, este recebe um limite de tempo para 
	 * ocupar o processador, e quando este tempo (tlcvruntime) é equivalente
	 * ao tempo que o processo já utilizou o processador (vruntime) então 
	 * este sai do processador e escolona um novo.
	 */
	if (curr_proc->maxvruntime == curr_proc->vruntime)
	{
		yield();
	}
